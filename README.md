...
| `asn [@nameserver] <ip\|host>`               | Looks up the origin ASN, BGP prefix, registry, allocation date, and AS name for an IPv4 address or hostname.                                                                                                |
| `calc [expression...]`                       | Evaluates arithmetic expressions. With no args, starts a REPL with `let name = expression` bindings.                                                                                                        |
